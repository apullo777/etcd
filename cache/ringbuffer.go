...
func (r *ringBuffer) Append(eventBatch []*clientv3.Event) {
	if len(eventBatch) == 0 {
		panic("ringBuffer.Append: empty group")
	}

	b := &batch{rev: eventBatch[0].Kv.ModRevision, events: eventBatch}
...
func (r *ringBuffer) Filter(minRev int64) (eventBatches [][]*clientv3.Event) {
	...
	for n, i := 0, r.tail; n < r.size; n, i = n+1, (i+1)%len(r.buffer) {
		...
		if batch.rev >= minRev {
			eventBatches = append(eventBatches, batch.events)
		}
	}
	return eventBatches
}
